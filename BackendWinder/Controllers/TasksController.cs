...
public class TasksController : ControllerBase
{
    ...
    public async Task<IActionResult> GetTasks()
    {
        try
        {
            ...
            // Если не мастер — только свои задания
            if (userRole != "master")
            {
                query = query.Where(t => t.WinderId == userId);
            }

            // 1. Сначала загружаем задания из БД
            var tasksFromDb = await query
                .OrderByDescending(t => t.CreatedAt)
                .ToListAsync();

            // 2. Собираем все ID мотальщиков
            var winderIds = tasksFromDb.Select(t => t.WinderId).Distinct().ToList();

            // 3. Загружаем имена пользователей из outside_db
        ...
        {
            _logger.LogError(ex, "Ошибка при получении заданий");
    ...
    public async Task<IActionResult> GetTaskById(int id)
    {
        ...
        {
            _logger.LogError(ex, $"Ошибка при получении задания {id}");
            return StatusCode(500, new { message = "Внутренняя ошибка сервера" });
        }
    }
    ...
    public async Task<IActionResult> UpdateStatus(int id, [FromBody] TaskStatusUpdateDto dto)
    {
        ...
        {
            _logger.LogError(ex, $"Ошибка при обновлении статуса задания {id}");
            return StatusCode(500, new { message = "Внутренняя ошибка сервера" });
        }
    }
    ...
    public async Task<IActionResult> DeleteTask(int id)
    {
        ...
        {
            _logger.LogError(ex, $"Ошибка при удалении задания {id}");
            return StatusCode(500, new { message = "Внутренняя ошибка сервера" });
        }
    }
    ...
    public async Task<IActionResult> CreateBatchTasks([FromBody] List<TaskCreateDto> items)
    {
        try
        {
            ...
            // ============================================================
            // ПРОВЕРКА ПРАВ:
            // - Мастер может создавать задания для любого мотальщика
            // - Мотальщик может создавать задания только для себя
            // ============================================================
            if (userRole != "master")
            {
                // Проверяем, что все задания назначены текущему пользователю
        ...
        {
            _logger.LogError(ex, "Ошибка при массовом создании заданий");
            return StatusCode(500, new { message = "Внутренняя ошибка сервера" });
        }
    }
}