...
<script>
import { brandManufsApi } from '../api/brandManufs';
...
export default {
  ...
  methods: {
    async loadItems() {
      try {
        const response = await brandManufsApi.getAll();
    ...
    async saveItem() {
      try {
        if (this.editingItem) {
          await brandManufsApi.update(this.editingItem.id, {
            id: this.editingItem.id,
            ...this.form,
          });
        } else {
          await brandManufsApi.create(this.form);
        }
        this.cancelForm();
    ...
    async deleteItem(id) {
      if (confirm('Удалить?')) {
        try {
          await brandManufsApi.delete(id);
          await this.loadItems();
        } catch (error) {
